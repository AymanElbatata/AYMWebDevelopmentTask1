...
namespace AYMWebDevelopment.Areas.AllMyAPI.Controllers
{
    ...
    public class RecoverPasswordController : ApiController
    {
        ...
        //        var data = mydb.RecoverPasswordTBL.Where(A => A.IsDeleted == false).ToList();
        ...
        //    {
        //        mydb.Configuration.ProxyCreationEnabled = false;
        //        var data = mydb.BranchTBL.Where(A => A.IsDeleted == false && A.BranchID == Id).SingleOrDefault();
        ...
        //    {
        //        mydb.Configuration.ProxyCreationEnabled = false;
        //        var data = mydb.BranchTBL.Where(A => A.IsDeleted == false && A.BranchName.StartsWith(StartSearchWord)).ToList();
        ...
        [HttpGet]
        public HttpResponseMessage RetrieveMultipleByEMPAutoCode(string EMPAutoCode)
        {
            try
            {
                mydb.Configuration.ProxyCreationEnabled = false;
                var data = mydb.RecoverPasswordTBL.Where(A => A.IsDeleted == false && A.RecoverPasswordAutoCode == EMPAutoCode).SingleOrDefault();
        ...
        [HttpPost]
        public HttpResponseMessage AddNewRow(RecoverPasswordTBL myobject, int Addnew, string APISAuthPinCode, string APISAuthPassword)
        {
            try
            {
                mydb.Configuration.ProxyCreationEnabled = false;
                ...
                if (isallowed)
                {
                    ...
                    var data = mydb.RecoverPasswordTBL.Add(myobject);
                    mydb.SaveChanges();
        ...
        [HttpPost]
        public HttpResponseMessage EditExistsRow(RecoverPasswordTBL myobject, int EditExists, string APISAuthPinCode, string APISAuthPassword)
        {
            try
            {
                mydb.Configuration.ProxyCreationEnabled = false;
                ...
                if (isallowed)
                {
                    ...
                    mydb.SaveChanges();
                    var data = mydb.RecoverPasswordTBL.Find(myobject.RecoverPasswordID);
                    string json = JsonConvert.SerializeObject(data);
                    return Request.CreateResponse(HttpStatusCode.Accepted, json);
                }
                else
        ...
        [HttpPost]
        public HttpResponseMessage StopExistsRow(RecoverPasswordTBL myobject, int Del, string APISAuthPinCode, string APISAuthPassword)
        {
            try
            {
                mydb.Configuration.ProxyCreationEnabled = false;
