...
{
    public class ResourceController : Base123Controller
    {
        // GET: AllMyMVC/Resource
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<ResourceTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(ResourceTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<ResourceTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(ResourceTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int ResourceID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + ResourceID).Result;
                var newobject = res1.Content.ReadAsAsync<ResourceTBL>().Result;
