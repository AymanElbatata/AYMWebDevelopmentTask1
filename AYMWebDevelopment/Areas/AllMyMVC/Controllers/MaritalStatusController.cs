...
{
    public class MaritalStatusController : Base123456Controller
    {
        // GET: AllMyMVC/MaritalStatus
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "MaritalStatus";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<MaritalStatusTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(MaritalStatusTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<MaritalStatusTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(MaritalStatusTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int MaritalStatusID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + MaritalStatusID).Result;
                var newobject = res1.Content.ReadAsAsync<MaritalStatusTBL>().Result;
