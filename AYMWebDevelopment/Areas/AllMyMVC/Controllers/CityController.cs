...
{
    public class CityController : Base123456Controller
    {
        // GET: AllMyMVC/City
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<CityTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(CityTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    if (model.CountryIDD == null || model.GovernorateIDD == null)
                    {
                        return Json(new { Result = "ERROR", Message = "Please Select Country/Governorate." });
                    }
                    //check if  exist
                    h.BaseAddress = new Uri(Link);
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<CityTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(CityTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int CityID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + CityID).Result;
                var newobject = res1.Content.ReadAsAsync<CityTBL>().Result;
