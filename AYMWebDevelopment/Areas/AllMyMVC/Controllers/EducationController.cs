...
{
    public class EducationController : Base123456Controller
    {
        // GET: AllMyMVC/Education
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "Education";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<EducationTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(EducationTBL model)
        {
            try
            {
        ...
        [HttpPost]
        public JsonResult UpdateOne(EducationTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int EducationID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + EducationID).Result;
                var newobject = res1.Content.ReadAsAsync<EducationTBL>().Result;
                ...
                if (newobject != null)
                {
                    ...
                    if (lst1 != "Canceled")
                    {
                        var result = JsonConvert.DeserializeObject<EducationTBL>(lst1);
                        return Json(new { Result = "OK" });
                    }
                    else
                    {
