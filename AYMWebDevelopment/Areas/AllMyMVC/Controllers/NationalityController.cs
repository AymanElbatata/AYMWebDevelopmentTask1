...
{
    public class NationalityController : Base123456Controller
    {
        // GET: AllMyMVC/Nationality
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "Nationality";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<NationalityTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(NationalityTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<NationalityTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(NationalityTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int NationalityID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + NationalityID).Result;
                var newobject = res1.Content.ReadAsAsync<NationalityTBL>().Result;
