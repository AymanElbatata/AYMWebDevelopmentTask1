...
{
    public class SalaryController : Base123Controller
    {
        // GET: AllMyMVC/Salary
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<SalaryTBL>>().Result.OrderBy(x=>x.SalaryAmount);
        ...
        [HttpPost]
        public JsonResult CreateOne(SalaryTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<SalaryTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(SalaryTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int SalaryID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + SalaryID).Result;
                var newobject = res1.Content.ReadAsAsync<SalaryTBL>().Result;
