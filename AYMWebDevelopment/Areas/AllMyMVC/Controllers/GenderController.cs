...
{
    public class GenderController : Base123456Controller
    {
        // GET: AllMyMVC/Gender
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<GenderTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(GenderTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<GenderTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(GenderTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int GenderID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + GenderID).Result;
                var newobject = res1.Content.ReadAsAsync<GenderTBL>().Result;
