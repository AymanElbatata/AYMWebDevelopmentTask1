...
{
    public class EmployeeLoginController : Base123Controller
    {
        // GET: AllMyMVC/EmployeeLogin
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int Searchtype = 0, DateTime? StartingDate = null, DateTime? EndingDate = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if ((txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch)) || (Searchtype > 0 && StartingDate != null && EndingDate != null))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<EmployeeLoginTBL>>().Result.OrderByDescending(x => x.LoginTime);
        ...
        //[HttpPost]
        //public JsonResult CreateOne(BranchTBL model)
        //{
        //    try
        //    {
        ...
        //                if (lst1 != "Canceled")
        //                {
        //                    var result = JsonConvert.DeserializeObject<BranchTBL>(lst1);
        ...
        //[HttpPost]
        //public JsonResult UpdateOne(BranchTBL model)
        //{
        //    try
        //    {
        //        if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int EmployeeLoginID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + EmployeeLoginID).Result;
                var newobject = res1.Content.ReadAsAsync<EmployeeLoginTBL>().Result;
