...
{
    public class BranchController : Base123456Controller
    {
        // GET: AllMyMVC/Branch
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<BranchTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult DeleteOne(int BranchID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + BranchID).Result;
                var newobject = res1.Content.ReadAsAsync<BranchTBL>().Result;
