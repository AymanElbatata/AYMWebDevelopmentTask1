...
{
    public class ExpenseRevenueTypeController : Base123Controller
    {
        // GET: AllMyMVC/ExpenseRevenueType
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "ExpenseRevenueType";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<ExpenseRevenueTypeTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(ExpenseRevenueTypeTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<ExpenseRevenueTypeTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(ExpenseRevenueTypeTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int ExpenseRevenueTypeID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + ExpenseRevenueTypeID).Result;
                var newobject = res1.Content.ReadAsAsync<ExpenseRevenueTypeTBL>().Result;
