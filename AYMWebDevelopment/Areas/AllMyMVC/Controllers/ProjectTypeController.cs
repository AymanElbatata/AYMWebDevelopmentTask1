...
{
    public class ProjectTypeController : Base123456Controller
    {
        // GET: AllMyMVC/ProjectType
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "ProjectType";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<ProjectTypeTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(ProjectTypeTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<ProjectTypeTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(ProjectTypeTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int ProjectTypeID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + ProjectTypeID).Result;
                var newobject = res1.Content.ReadAsAsync<ProjectTypeTBL>().Result;
