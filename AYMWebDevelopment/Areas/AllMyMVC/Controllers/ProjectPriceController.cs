...
{
    public class ProjectPriceController : Base123456Controller
    {
        // GET: AllMyMVC/ProjectPrice
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "ProjectPrice";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<ProjectPriceTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(ProjectPriceTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<ProjectPriceTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(ProjectPriceTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int ProjectPriceID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + ProjectPriceID).Result;
                var newobject = res1.Content.ReadAsAsync<ProjectPriceTBL>().Result;
