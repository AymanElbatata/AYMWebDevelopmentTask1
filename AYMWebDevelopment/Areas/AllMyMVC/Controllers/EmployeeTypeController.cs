...
{
    public class EmployeeTypeController : Base123Controller
    {
        // GET: AllMyMVC/EmployeeType
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "EmployeeType";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<EmployeeTypeTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(EmployeeTypeTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<EmployeeTypeTBL>(lst1);
        ...
        //[HttpPost]
        //public JsonResult UpdateOne(EmployeeTypeTBL model)
        //{
        //    try
        //    {
        //        if (ModelState.IsValid)
        ...
        //[HttpPost]
        //public JsonResult DeleteOne(int EmployeeTypeID)
        //{
        //    try
        //    {
        //        h.BaseAddress = new Uri(Link);
        //        var res1 = h.GetAsync(API + "?Id=" + EmployeeTypeID).Result;
        //        var newobject = res1.Content.ReadAsAsync<EmployeeTypeTBL>().Result;
