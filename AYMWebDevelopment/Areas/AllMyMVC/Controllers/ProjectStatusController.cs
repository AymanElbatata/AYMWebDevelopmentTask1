...
{
    public class ProjectStatusController : Base123456Controller
    {
        // GET: AllMyMVC/ProjectStatus
        AYMCOMPDataContext mydb = new AYMCOMPDataContext();

        string Link = APILinks.GetAPILINK(1);
        string API = "ProjectStatus";
        ...
        public JsonResult listAll(string txtENBTNSearch = null, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                if (txtENBTNSearch != null && txtENBTNSearch != "" && !string.IsNullOrEmpty(txtENBTNSearch))
                {
                    ...
                    var lst = res.Content.ReadAsAsync<List<ProjectStatusTBL>>().Result;
        ...
        [HttpPost]
        public JsonResult CreateOne(ProjectStatusTBL model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (reqexist == 0)
                    {
                        ...
                        if (lst1 != "Canceled")
                        {
                            var result = JsonConvert.DeserializeObject<ProjectStatusTBL>(lst1);
        ...
        [HttpPost]
        public JsonResult UpdateOne(ProjectStatusTBL model)
        {
            try
            {
                if (ModelState.IsValid)
        ...
        [HttpPost]
        public JsonResult DeleteOne(int ProjectStatusID)
        {
            try
            {
                h.BaseAddress = new Uri(Link);
                var res1 = h.GetAsync(API + "?Id=" + ProjectStatusID).Result;
                var newobject = res1.Content.ReadAsAsync<ProjectStatusTBL>().Result;
